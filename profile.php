<?php
 ...
 if (isset($_POST['upload_profile_pic'])) {
  ...
  $sql_update_pic = "UPDATE useraccount SET profile_picture = '$target_file' WHERE id = '$user_id'";
  ...
  echo "<script>alert('Error updating profile picture in database: " . mysqli_error($conn) . "');</script>";
  }
  } else {
 ...
 if ($result_user && mysqli_num_rows($result_user) > 0) {
  ...
  $profile_picture = $user['profile_picture']; // Fetch the profile picture path
 } else {
  // Handle if user not found (optional)
  $email = $name = $age = $contact = $usertype = $profile_picture = '';
 }
 ...
 <html lang="en">
  ...
  cursor: pointer;
  margin-bottom: 30px; /* Added margin below the clickable area */
  }
  .profile-details {
  display: grid;
  ...
  <img id="profile-picture" src="<?php echo htmlspecialchars($profile_picture); ?>" alt="Profile Picture" class="profile-picture">
  </label>
  <input type="file" name="profile_pic" id="profile-pic" class="profile-pic-input" accept="image/*" style="display: none;" onchange="this.form.submit()">
  ...
  // No need for previewImage function anymore, form submits on file change
  </script>
  </body>
  </html>